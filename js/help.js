...
app.post('/writedone', function (request, response) {
	...
	const sql =
		'INSERT INTO HELP (name, timecoin, gender,title, content, address) VALUES (?,?,?,?,?,?)';
...
app.post('/modifydone', (request, response) => {
	...
	let sql =
		"update help set name='${name}', timecoin='${timecoin}', gender='${gender}', title='${title}', content='${content}', address='${address}', today=now() where idx='${idx}'";
